  ...
  try {
   ...
   if(credential) {
    ...
    if (user) {
      ...
      const nuevoUsuario = await Usuario.create({ nombre: dataUsuario.given_name, apellido: dataUsuario.family_name, imagen: dataUsuario.picture, email: dataUsuario.email, rol: "usuario"});
      const token = jwt.sign({ id: nuevoUsuario.id }, process.env.JWT_SECRET, { expiresIn: '1h' });
      res.json({token, user: nuevoUsuario});
    }
   }
